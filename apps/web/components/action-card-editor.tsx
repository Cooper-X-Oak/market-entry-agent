...
export function ActionCardEditor({ missionId, card }: { missionId: string; card: ActionCard }) {
  ...
  async function save() { setSaving(true); try { const next = await apiClient<ActionCard>(`/api/v1/missions/${missionId}/action-cards/${card.id}`, { method: 'PATCH', body: JSON.stringify({ ...form, expectedVersionNo: card.versionNo }) }); router.push(`/missions/${missionId}/action-queue/${next.id}`); router.refresh(); } finally { setSaving(false); } }
