...
import { from, map, mergeMap, of, type Observable } from 'rxjs';
...
export class IdempotencyInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<unknown>> {
    ...
    const [existing] = await this.transactions.run(auth.tenantId, (tx) => tx.select().from(idempotencyRecords).where(and(eq(idempotencyRecords.tenantId, auth.tenantId), eq(idempotencyRecords.endpoint, endpoint), eq(idempotencyRecords.idempotencyKey, key))).limit(1));
    ...
    if (existing) await this.transactions.run(auth.tenantId, (tx) => tx.delete(idempotencyRecords).where(eq(idempotencyRecords.id, existing.id)));

    return next.handle().pipe(mergeMap((data: unknown) => from(this.transactions.run(auth.tenantId, (tx) => tx.insert(idempotencyRecords).values({ tenantId: auth.tenantId, idempotencyKey: key, endpoint, requestHash, responseStatus: 200, responseBody: data, expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000) }).onConflictDoNothing())).pipe(map(() => data))));
  }
}
