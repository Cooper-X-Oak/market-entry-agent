...
const routeReviewSchema = z.object({ approvedRouteIds: z.array(z.uuid()).min(1).max(3), acceptedArtifactVersionIds: z.array(z.uuid()).min(1).max(3), comment: z.string().optional() });
const capabilityReviewSchema = z.object({ resolvedClaimIds: z.array(z.uuid()).min(1), comment: z.string().optional() });
...
export class MissionsController {
  ...
  @Get(':missionId/cockpit') cockpit(@CurrentAuth() auth: AuthContext, @Param('missionId') missionId: string) { return this.service.cockpit(auth, missionId); }
  @Get(':missionId/export') async exportMission(@CurrentAuth() auth: AuthContext, @Param('missionId') missionId: string, @Res() reply: FastifyReply) { const result = await this.exporter.create(auth, missionId); return reply.header('Content-Type', 'application/zip').header('Content-Disposition', downloadDisposition(result.filename)).send(result.bytes); }
  ...
  @Post(':missionId/capabilities/review-complete') completeCapabilityReview(@CurrentAuth() auth: AuthContext, @Param('missionId') missionId: string, @Body(new ZodPipe(capabilityReviewSchema)) body: z.infer<typeof capabilityReviewSchema>) { return this.service.completeCapabilityReview(auth, missionId, body); }
