...
const targetReviewSchema = z.object({ selectedTargetIds: z.array(z.uuid()).min(1).max(3), comment: z.string().optional() });
...
export class EcosystemController {
  ...
  @Post('targets/review-complete') completeTargetReview(@CurrentAuth() auth: AuthContext, @Param('missionId') missionId: string, @Body(new ZodPipe(targetReviewSchema)) body: z.infer<typeof targetReviewSchema>) { return this.service.completeTargetReview(auth, missionId, body); }
