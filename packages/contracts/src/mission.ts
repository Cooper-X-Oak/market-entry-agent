...
export const budgetConfigSchema = z.object({
  maxTargets: z.number().int().min(10).max(100).default(40),
  maxContactPaths: z.number().int().min(1).max(1000).default(160),
  maxSearchCalls: z.number().int().min(20).max(300).default(80),
  maxBrowserPages: z.number().int().min(30).max(500).default(120),
  maxAgentRuns: z.number().int().min(50).max(800).default(200),
  maxModelTokens: z.number().int().min(10_000).default(1_000_000),
...
export const missionBriefSchema = z.object({
  company: z.object({ name: z.string(), website: z.url(), productSummary: z.string() }),
