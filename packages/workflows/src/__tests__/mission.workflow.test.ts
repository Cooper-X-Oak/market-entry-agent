...
describe('mission workflow replay', () => {
  ...
  afterAll(async () => { if (environment) await environment.teardown(); });

  async function runFixture(supplierKnown = true): Promise<string[]> {
    const trace: string[] = [];
    let completeAtRouteReview: () => Promise<void> = () => Promise.resolve();
    ...
    const activities = {
      loadMission: () => {
        ...
        return Promise.resolve({ id: missionId, supplierKnown, topTargetLimit: 10, budget: { maxTargets: 20, maxContactPaths: 160, maxSearchCalls: 80, maxBrowserPages: 120, maxAgentRuns: 200, maxModelTokens: 1_000_000, weeklyRefreshTargets: 20 } });
      },
      ...
      markMissionAwaitingCapabilityReview: () => { trace.push('markMissionAwaitingCapabilityReview'); return Promise.resolve(); },
      recordCapabilityReview: () => { trace.push('recordCapabilityReview'); return Promise.resolve(); },
      ...
      markMissionAwaitingRouteReview: () => { trace.push('markMissionAwaitingRouteReview'); return completeAtRouteReview(); },
    ...
    completeAtRouteReview = () => handle.signal(missionCompletionRequested, { requestedByUserId: userId });
    await handle.signal(capabilityReviewSubmitted, {
      resolvedClaimIds: ['31000000-0000-4000-8000-000000000004'],
  ...
  it('does not retry a failed business output Activity', async () => {
    let attempts = 0;
    const taskQueue = `invalid-output-${crypto.randomUUID()}`;
    const worker = await Worker.create({ connection: environment.nativeConnection, taskQueue, workflowsPath: fileURLToPath(new URL('../workflow-entry.ts', import.meta.url)), activities: {
      loadMission: () => Promise.resolve({ id: missionId, supplierKnown: false, topTargetLimit: 10, budget: {} }),
      compileMission: () => { attempts++; return Promise.reject(ApplicationFailure.nonRetryable('Controlled output failure', 'OUTPUT_INVALID')); },
    } });
    const handle = await environment.client.workflow.start(missionWorkflow, { taskQueue, workflowId: `invalid-output-${crypto.randomUUID()}`, args: [{ tenantId, missionId }] });
    await expect(worker.runUntil(handle.result())).rejects.toThrow();
    expect(attempts).toBe(1);
  });

  it('skips supplier capability research and review when the supplier is unknown', async () => {
    const trace = await runFixture(false);
    expect(trace).toContain('researchMarketRoutes');
    expect(trace).not.toContain('ingestCompanySources');
    expect(trace).not.toContain('extractCapabilityClaims');
    expect(trace).not.toContain('markMissionAwaitingCapabilityReview');
  });
